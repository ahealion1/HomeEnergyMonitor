%% Audio Classification - 4 Random Forest (Background Noise - Microwave)
% Alex Healion - C15399731
                                                         %#ok<*SAGROW,*ASGLU,*NASGU,*NBRAK>
%% Run Data Processing and Compilation
load('microwavedata1.mat');                                         % Load in training data without test data
...
test_1_resultsRFmicrowave = zeros(param_loop_num,loop_num+2);       % Pre-allocating the array for speed
test_2_resultsRFmicrowave = zeros(param_loop_num,loop_num+2);
test_3_resultsRFmicrowave = zeros(param_loop_num,loop_num+2);
test_4_resultsRFmicrowave = zeros(param_loop_num,loop_num+2);
test_1_opRFmicrowave = cell(param_loop_num,loop_num);               % Pre-allocating the array for speed
test_2_opRFmicrowave = cell(param_loop_num,loop_num);
test_3_opRFmicrowave = cell(param_loop_num,loop_num);
test_4_opRFmicrowave = cell(param_loop_num,loop_num);
...
for param_loop = 1:param_loop_num                                   % Parameter testing for loop
    for loop = 1:loop_num                                           % Verification for loop
        ...
        % Test 1
        [testaudio1,fs] = audioread('microwave_10cm_2.mp3');      % Read in test audio
        ...
        test_1_opRFmicrowave{param_loop,loop} = (predict(baggedtree,test_comp.')).';    % Input test features into random forest to get outputs
        test_1_resultsRFmicrowave(param_loop,loop) = sum(count(test_1_opRFmicrowave{param_loop,loop},'microwave'))/length(test_1_opRFmicrowave{param_loop,loop});    % Get output results
        test_1_resultsRFmicrowave(param_loop,loop_num+2) = (sum(test_1_resultsRFmicrowave(param_loop,1:loop_num))/loop_num);  % Get average of results
        clear test test_cent test_benergy test_flux test_rollofffreq test_MFCCoeffs test_comp;  % Clear unwanted variables
                                                                    % Same for other tests
        % Test 2
        [testaudio2,fs] = audioread('f_microwave_2.mp3');
        ...
        test_2_opRFmicrowave{param_loop,loop} = (predict(baggedtree,test_comp.')).';
        test_2_resultsRFmicrowave(param_loop,loop) = sum(count(test_2_opRFmicrowave{param_loop,loop},'microwave'))/length(test_2_opRFmicrowave{param_loop,loop});
        test_2_resultsRFmicrowave(param_loop,loop_num+2) = (sum(test_2_resultsRFmicrowave(param_loop,1:loop_num))/loop_num);
        clear test test_cent test_benergy test_flux test_rollofffreq test_MFCCoeffs test_comp;

        % Test 3
        [phonetestaudio3,phonefs] = audioread('phone_microwave2.wav');
        ...
        test_3_opRFmicrowave{param_loop,loop} = (predict(baggedtree,phonetest_comp.')).';
        test_3_resultsRFmicrowave(param_loop,loop) = sum(count(test_3_opRFmicrowave{param_loop,loop},'microwave'))/length(test_3_opRFmicrowave{param_loop,loop});
        test_3_resultsRFmicrowave(param_loop,loop_num+2) = (sum(test_3_resultsRFmicrowave(param_loop,1:loop_num))/loop_num);
        ...
        % Test 4
        [phonetestaudio4,phonefs] = audioread('phone_microwave1.wav');
        ...
        test_4_opRFmicrowave{param_loop,loop} = (predict(baggedtree,phonetest_comp.')).';
        test_4_resultsRFmicrowave(param_loop,loop) = sum(count(test_4_opRFmicrowave{param_loop,loop},'microwave'))/length(test_4_opRFmicrowave{param_loop,loop});
        test_4_resultsRFmicrowave(param_loop,loop_num+2) = (sum(test_4_resultsRFmicrowave(param_loop,1:loop_num))/loop_num);
        clear phonetest_stereo phonetest phonetest_cent phonetest_benergy phonetest_flux phonetest_rollofffreq phonetest_MFCCoeffs phonetest_comp;
...
resultssummaryRF = [test_1_resultsRFmicrowave(:,end) test_2_resultsRFmicrowave(:,end) test_3_resultsRFmicrowave(:,end) test_4_resultsRFmicrowave(:,end)];
