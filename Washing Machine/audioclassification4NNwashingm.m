%% Audio Classification - 4 Neural Network (Background Noise - Washing Machine)
...
test_1_resultsNNwashingm = zeros(param_loop_num,loop_num+2);        % Pre-allocating the array for speed
test_2_resultsNNwashingm = zeros(param_loop_num,loop_num+2);
test_3_resultsNNwashingm = zeros(param_loop_num,loop_num+2);
test_4_resultsNNwashingm = zeros(param_loop_num,loop_num+2);
test_1_opNNwashingm = cell(param_loop_num,loop_num);                % Pre-allocating the array for speed
test_2_opNNwashingm = cell(param_loop_num,loop_num);
test_3_opNNwashingm = cell(param_loop_num,loop_num);
test_4_opNNwashingm = cell(param_loop_num,loop_num);
...
for param_loop = 1:param_loop_num                                   % Parameter testing for loop
    for loop = 1:loop_num                                           % Verification for loop
        ...
        test_1_opNNwashingm{param_loop,loop} = trained_net(test_comp,'useParallel','yes');    % Input test features into trained network to get outputs
        test_1_resultsNNwashingm(param_loop,loop) = (sum(test_1_opNNwashingm{param_loop,loop}(2,:))/length(test_1_opNNwashingm{param_loop,loop}));    % Get output results
        test_1_resultsNNwashingm(param_loop,loop_num+2) = (sum(test_1_resultsNNwashingm(param_loop,1:loop_num))/loop_num);  % Get average of results
        ...
        test_2_opNNwashingm{param_loop,loop} = trained_net(test_comp,'useParallel','yes');
        test_2_resultsNNwashingm(param_loop,loop) = (sum(test_2_opNNwashingm{param_loop,loop}(2,:))/length(test_2_opNNwashingm{param_loop,loop}));
        test_2_resultsNNwashingm(param_loop,loop_num+2) = (sum(test_2_resultsNNwashingm(param_loop,1:loop_num))/loop_num);
        ...
        test_3_opNNwashingm{param_loop,loop} = trained_net(phonetest_comp,'useParallel','yes');
        test_3_resultsNNwashingm(param_loop,loop) = (sum(test_3_opNNwashingm{param_loop,loop}(2,:))/length(test_3_opNNwashingm{param_loop,loop}));
        test_3_resultsNNwashingm(param_loop,loop_num+2) = (sum(test_3_resultsNNwashingm(param_loop,1:loop_num))/loop_num);
        ...
        test_4_opNNwashingm{param_loop,loop} = trained_net(phonetest_comp,'useParallel','yes');
        test_4_resultsNNwashingm(param_loop,loop) = (sum(test_4_opNNwashingm{param_loop,loop}(2,:))/length(test_4_opNNwashingm{param_loop,loop}));
        test_4_resultsNNwashingm(param_loop,loop_num+2) = (sum(test_4_resultsNNwashingm(param_loop,1:loop_num))/loop_num);
        clear phonetest_stereo phonetest phonetest_cent phonetest_benergy phonetest_flux phonetest_rollofffreq phonetest_MFCCoeffs phonetest_comp;
...
resultssummaryNN = [test_1_resultsNNwashingm(:,end) test_2_resultsNNwashingm(:,end) test_3_resultsNNwashingm(:,end) test_4_resultsNNwashingm(:,end)];
